...
class TestScaffoldCLIValidDomains:
    ...
    def test_valid_domain_creates_files(self, domain):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(py_files) >= 3, f"Expected at least 3 Python files for {domain}, got {len(py_files)}"
    ...
    def test_valid_domain_with_flags(self, domain):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Should succeed
            assert result.returncode == 0, f"Failed for {domain} with flags: {result.stdout}\n{result.stderr}"
...
class TestScaffoldCLIFlagBehavior:
    ...
    def test_no_repository_flag(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                ["poetry", "run", "fin-infra", "budgets", "--dest-dir", str(dest_dir), "--no-with-repository"],
                capture_output=True,
                text=True,
            )
